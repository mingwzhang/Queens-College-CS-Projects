...
// java Membership -firstname=Steve -lastname=Smith -id=1234567890 -age=23 -NewYork=false -outputFile=_output1.txt -inputFile=_inputMember.txt -outputMode=all
// java Membership -firstname=Steve -lastname=Smith -id=1234567890 -age=23 -NewYork=false -outputFile=_output2.txt -inputFile=_inputMember.txt -outputMode=current
...
public class Membership {
    ...
    public static void main(String[] args) {
        ...
        String outputMode = "current"; // Default mode is current member only
        ...
        for (String arg : args) {
            ...
            else if (arg.startsWith("-outputMode=")) outputMode = arg.split("=")[1]; // New output mode option
        ...
        if (outputFile != null) {
            createOutputFile(outputFile, outputMode, newMember);
        }
    }
    ...
    /**
     * Saves data to an output file based on the outputMode.
     * @param fileName The name of the output file.
     * @param outputMode Determines whether to print only the new member or all members.
     * @param newMember The new member being added.
     */
    private static void createOutputFile(String fileName, String outputMode, Member newMember) {
        try (BufferedWriter bw = new BufferedWriter(new FileWriter(fileName))) {
            if ("all".equals(outputMode)) {
                // Write all members
                ...
                // Write only the new member
                bw.write(newMember.firstName + "|" + newMember.lastName + "|" + newMember.id + "|" + newMember.age + "|" + newMember.livesInNewYork);
                bw.newLine();
            }
            System.out.println("Output file " + fileName + " created successfully with mode: " + outputMode);
        } catch (IOException e) {
